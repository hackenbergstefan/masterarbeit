...
\begin{proof}[von \thref{satz:scheerhorn1}]
  ...
  und da $D_n(x,a)-b$ irreduzibel nach Voraussetzung, hat
...
\begin{bemerkung}
  ...
  $\theta^l+a^l\theta^{-l}$ über $F$ hat: Es gilt nämlich offenbar
  \[ D_{\frac{n}{\ggT(n,l)}}(\theta^l+a^l\theta^{-l},a)-b = 0\,. \]
\end{bemerkung}
...
Daher ist das Resultat vielleicht auch nicht besonders überraschend und in
Notation des vorherigen Abschnittes würde man die Aussagen vielleicht auf die
folgende Art und Weise aufschreiben.

\begin{satz}
  Seien $F := \F_q$ ein endlicher Körper von Charakteristik $p$ und $m\in
  \N^\ast$ ungerade mit $p\nmid m$ und $\nu(m)\mid q^2-1$. Sind $a,b\in F$, 
  sodass $D_m(x,a)-b \in F[x]$ irreduzibel mit Nullstelle $\gamma \in E:=
  \F_{q^m}$, so gilt:
  \begin{enumerate}
    \item $E = F(\gamma+a\gamma\inv)$,
    \item Für alle $l\in R_q(m)$ mit $r_q(l\bmod m) = \ord_q(m)$ ist 
      $\Ord_q(D_l(\gamma+a\gamma\inv,a))$ ist ein irreduzibler Teiler von
      $\Phi_m(x)$.
    \item Setze 
      \[ v := \sum_{l\in R_q(m)} D_l(\gamma+a\gamma\inv,a)\,. \]
      Dann ist $v \in E$ normal über $F$.
  \end{enumerate}
\end{satz}
