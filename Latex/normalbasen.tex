...
also Nullstellen der Kreisteilungspolynomen, bedient,
...
da wir in \thref{satz:q_ordnung_von_trace} erkannt haben, wie die Spurfunktion
...
\begin{satz}
  ...
  \begin{enumerate}
    ...
    \item Ist $\nu(n)\mid (q+1)$, so gilt:
      ...
      $\Ord_q(D_l(\gamma,a))$ ist ein irreduzibler monischer Teiler von
      $\Phi_m(x)$.
    \item Setze 
...
\begin{proof}
  ...
  \[ \alpha \speq= E_{n-1}(t,a) + \sum_{i=1}^{n-1} E_{n-1-i}(t,a)
    (\theta^ia^i+\theta^{-i})\,.\]
  Die Aussage folgt dann sofort mit \thref{satz:scheerhorn1} 
