...
inline void matmul(struct FFElem **mat, struct FFElem *ff,
    ...
    int i,j,i2, row;
    bool end;
    /*printf("matmul mat:");printFFElemMatrix(mat,m);*/
...
inline int testAllSubmods(struct FFElem *ff, struct FFElem *mipo, 
    ...
    for(i=0;i<decompCount;i++){
        ...
        if(goodCounter == polysCountPerDecomp[i]) return i;
        curDecompPosition += polysCountPerDecomp[i];
    }
    return -1;
}
...
inline bool testSubmod(struct FFElem *ff, struct FFElem *mipo, 
        ...
        struct FFElem **mats, int frobPower,
        int m, int *tmp, 
        struct FFElem *ffTmp, struct FFElem *ffTmp2, struct FFElem *ffTmp3, 
        struct FFElem **matmulCache, bool *matmulCacheCalced,
        int *multTable, int *addTable){
    ...
    for(j=0;j<polysCount;j++){
        applyFrob(ff,mipo,
                polys[j],
                mats,frobPower, ffTmp,
                m,tmp,ffTmp2,ffTmp3,
                matmulCache,matmulCacheCalced,
                multTable,addTable);
        if( isZero(ffTmp) == evalToZero[j] ){
            goodCounter++;
        }else return false;
    }
    if(goodCounter == polysCount) return true;
    return false;
}
...
inline void calcSubmoduleElements(struct Node *root,
        ...
        struct FFElem **mats, int matLen, int frobPower,
...
unsigned long long processFiniteField(struct FFElem *mipo, int decompCount,
    ...
    int i,j;
    //correct multTable and addTable
...
void main(){
    ...
    /*multiplyFFElem(x,x2,ret,mipo,tmp,m,multTable,addTable);*/
    /*printFFElem("ret", ret);*/
